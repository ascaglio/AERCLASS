...
def propagate_methodII(df, aod_error):
    ...
    return propagate_methodI(df, aod_error).assign(arod_sub = np.maximum(df['arod'] - arod_error,0),
    arod_sob = df['arod'] + arod_error)
...
def propagate_methodIV(df, aod_error, ssa_error):
    return propagate_methodI(df, aod_error).assign(
        ssa440_sub = np.maximum(df['ssa440'] - ssa_error,0),
        ssa440_sob = df['ssa440'] + ssa_error
    )
...
def propagate_methodV(df, aod_error, ssa_error):
    ...
    return propagate_methodI(df, aod_error).assign(aae_sub = np.maximum(df['aae'] - aae_error,0),
    aae_sob = df['aae'] + aae_error
    )


def propagate_methodVI(df, aod_error, rri_error):
    return propagate_methodI(df, aod_error).assign(
        rri440_sub = np.maximum(df['rri440'] - rri_error,0),
        rri440_sob = df['rri440'] + rri_error
    )
